...
TEST_CASE("PaddleMotion::stepCenterY: axis = +1 moves down at speed * dt")
{
    ...
    const float next = PaddleMotion::stepCenterY(1.0f, start, kHalfHeight, kSpeed, kPlayfieldHeight, dt);
...
{
    const float start = 300.0f;
    const float next = PaddleMotion::stepCenterY(0.0f, start, kHalfHeight, kSpeed, kPlayfieldHeight, 1.0 / 60.0);
...
TEST_CASE("PaddleMotion::stepCenterY: a single huge-dt step cannot escape the playfield bounds")
{
    ...
    const float bottomBound = kPlayfieldHeight - kHalfHeight;
    const float next = PaddleMotion::stepCenterY(1.0f, 300.0f, kHalfHeight, kSpeed, kPlayfieldHeight, 60.0);
    CHECK(next == doctest::Approx(bottomBound));

    const float prev = PaddleMotion::stepCenterY(-1.0f, 300.0f, kHalfHeight, kSpeed, kPlayfieldHeight, 60.0);
...
TEST_CASE("PaddleMotion::stepCenterY: an out-of-range axis is still capped by speed * dt")
{
    ...
    const float start = 300.0f;
    const double dt = 1.0 / 60.0;
    const float next = PaddleMotion::stepCenterY(2.0f, start, kHalfHeight, kSpeed, kPlayfieldHeight, dt);
    CHECK((next - start) == doctest::Approx(kSpeed * static_cast<float>(dt)));
}
...
TEST_CASE("PaddleMotion::stepCenterY: one big dt step equals N small dt steps of equal sum (framerate independence)")
{
    ...
    const float bigStep = PaddleMotion::stepCenterY(1.0f, start, kHalfHeight, kSpeed, kPlayfieldHeight, totalSeconds);

    float manyStepsCenterY = start;
    for (int i = 0; i < steps; ++i)
    {
        manyStepsCenterY =
            PaddleMotion::stepCenterY(1.0f, manyStepsCenterY, kHalfHeight, kSpeed, kPlayfieldHeight, smallDt);
    }
