...
float stepCenterY(const Paddle& paddle, float axis, float playfieldHeight, double dtSeconds)
{
    // Defend at the boundary: negative `paddle.speed` or negative `dtSeconds` would make `maxStep` negative, and
    ...
    const float maxStep = std::max(0.0f, paddle.speed * static_cast<float>(dtSeconds));
    ...
    return clampCenterY(paddle.centerY + cappedDelta, paddle.halfHeight, playfieldHeight);
}
