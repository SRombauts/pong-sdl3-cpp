...
// Owns the rectangles for the static parts of the playfield (today: the dashed vertical center line) and writes them
...
// chrome that stays static across the project's lifetime (center line today; potentially border frames, score
// backings, etc. once the menus milestone lands).
class PlayfieldRenderer
{
public:
    // Pre-compute every cached rect from the playfield dimensions and the center-line tuning. Parameters mirror
    // PlayfieldLayout::centerDashSegments by design so the class stays testable in isolation, without baking the
    ...
    // Issue one SDL_RenderFillRect per cached dash. Does not touch the renderer's draw-color state, so the caller can
    ...
    [[nodiscard]] const std::vector<SDL_FRect>& centerDashes() const noexcept
    {
        return m_centerDashes;
    }

private:
    std::vector<SDL_FRect> m_centerDashes;
};
